...
If `npm install` previously failed with `403` responses from the npm registry, the checked-in `.npmrc` disables the
problematic corporate proxy variables and forces npm to talk straight to `https://registry.npmjs.org`. That allows the
install to succeed inside this environment without having to manually unset the proxy-related environment variables.
